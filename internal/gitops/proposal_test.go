...
func TestPullRequestServiceCreatesGitHubPullRequest(t *testing.T) {
	...
	old := newPullRequestHTTPClient
	defer func() { newPullRequestHTTPClient = old }()
	newPullRequestHTTPClient = func() *http.Client {
	...
	result, err := PullRequestService{client: newPullRequestHTTPClient()}.Create(context.Background(), PullRequestRequest{
		Provider: "github",
