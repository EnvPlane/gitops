...
func (w *RepositoryWriter) prepare(ctx context.Context) error {
	...
	if w.usesWorkBranch() {
		if err := runGitWithSecret(ctx, w.target.Workspace, w.target.SecretValue, "fetch", "origin", w.target.PushBranch); err == nil {
			if err := runGit(ctx, w.target.Workspace, "checkout", "-B", w.target.PushBranch, "origin/"+w.target.PushBranch); err != nil {
				return err
			}
		} else {
...
func validateRepositoryTarget(rawURL, path, branch, pushBranch string, allowLocalRepository bool) error {
	...
	if err != nil || (parsed.Scheme != "https" && parsed.Scheme != "ssh" && !strings.HasPrefix(rawURL, "git@") && !(allowLocalRepository && filepath.IsAbs(rawURL)) && !isExplicitLocalURL) {
		return fmt.Errorf("gitops repository url must use https, ssh, scp, or an explicit file URL")
	}
